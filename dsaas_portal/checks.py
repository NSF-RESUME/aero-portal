    ...
    if not any(p["uuid"] for p in settings.SEARCH_INDEXES.values()):
        errors.append(
            Info(
                ...
                'globus search index create dsaas_portal "A search index for DSaaS" '
                "And set the UUID inside dsaas_portal.apps.SEARCH_INDEXES[].uuid",
                obj=settings,
                id="dsaas_portal.W002",
            )
        )
    return errors
